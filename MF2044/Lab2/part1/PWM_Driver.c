 ...
 *  ------------------------------------------------------------------------- 
 *
 * Created: Tue Apr 17 17:54:40 2018
 */

#define S_FUNCTION_LEVEL 2
#define S_FUNCTION_NAME PWM_Driver
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
...
{
    const real_T *duty = (const real_T *) ssGetInputPortSignal(S, 0);
    const real_T *xD = ssGetDiscStates(S);

    PWM_Driver_Outputs_wrapper(duty, xD);
...
static void mdlUpdate(SimStruct *S, int_T tid)
{
    ...
    PWM_Driver_Update_wrapper(duty, xD);
}
