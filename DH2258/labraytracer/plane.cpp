...
bool PlaneX::closestIntersection(const Ray& ray, double maxLambda,
    ...
    double d = dot(ray.getDirection(), normal_);	// compute dot product

	// false if absolute value of d is < epsilon
    if (fabs(d) < Util::epsilon) {
        return false;
    }

    double a = dot((point_ - ray.getOrigin()), normal_);	// "point_" is plane distance from scene origin

    double lambda = a / d;	// Projection of our ray onto the normal
